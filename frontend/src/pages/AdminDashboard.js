...
import { Calendar, Check, Camera, Mic, Tv, X, Clock, Users, FileText, Image, Video, Plus, Minus, User, LogOut, Upload, Loader, Download, Eye, Printer, ArrowLeft, Shield, Trash2, MessageSquare, FileArchive, AlertTriangle, CreditCard, Phone, ClipboardList, Heart, MessageCircle, Music, BookOpen, QrCode, Copy, HardDrive, Play, Settings, Maximize, Link } from "lucide-react";
        ...
        {activeTab === "kiosk" && (
          <div data-testid="admin-kiosk-tab">
            ...
            <div className="mt-8 bg-white/5 rounded-lg p-6">
              ...
              <p className="text-white/60 text-sm mb-4">
                Configurez les prix selon le format et le type de cadre
              </p>
                ...
                <div className="space-y-6">
                  {photofindEvents.map(evt => (
                    <div key={evt.id} className="bg-white/5 rounded-lg p-4">
                      ...
                      {/* Print Format Pricing Table */}
                      <div className="mb-6">
                        <h5 className="font-bold text-blue-400 mb-3 flex items-center gap-2">
                          <Printer size={16} /> Tarifs Impression par Format
                        </h5>
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b border-white/20">
                                ...
                                <th className="text-center py-2 px-3">Sans cadre</th>
                                <th className="text-center py-2 px-3">Avec cadre</th>
                              </tr>
                            </thead>
                            <tbody>
                              {[
                                { id: "10x15", name: "10 x 15 cm" },
                                { id: "13x18", name: "13 x 18 cm" },
                                { id: "15x20", name: "15 x 20 cm" },
                                { id: "20x30", name: "20 x 30 cm" },
                                { id: "A4", name: "A4" },
                                { id: "A5", name: "A5" }
                              ].map(format => (
                                <tr key={format.id} className="border-b border-white/10">
                                  ...
                                  <td className="py-2 px-3">
                                    <div className="flex items-center justify-center gap-1">
                                      <input
                                        type="number"
                                        step="0.5"
                                        value={evt.pricing?.formats?.[format.id]?.sans_cadre ?? (format.id === "10x15" ? 5 : format.id === "A4" ? 20 : 10)}
                                        onChange={(e) => updateFormatPricing(evt.id, format.id, 'sans_cadre', parseFloat(e.target.value))}
                                        className="w-16 bg-white/10 border border-white/20 px-2 py-1 rounded text-center"
                                      />
                                  ...
                                  <td className="py-2 px-3">
                                    <div className="flex items-center justify-center gap-1">
                                      <input
                                        type="number"
                                        step="0.5"
                                        value={evt.pricing?.formats?.[format.id]?.avec_cadre ?? (format.id === "10x15" ? 8 : format.id === "A4" ? 25 : 15)}
                                        onChange={(e) => updateFormatPricing(evt.id, format.id, 'avec_cadre', parseFloat(e.target.value))}
                                        className="w-16 bg-white/10 border border-white/20 px-2 py-1 rounded text-center"
                                      />
                                      <span className="text-white/50">€</span>
                                    </div>
                      ...
                      <div className="mb-6">
                        <h5 className="font-bold text-green-400 mb-3 flex items-center gap-2">
                          <Mail size={16} /> Tarifs Email (HD numérique)
                        </h5>
                      ...
                      <button
                        ...
                        className="bg-primary text-black px-6 py-2 font-bold hover:bg-primary/90 rounded"
                      >
